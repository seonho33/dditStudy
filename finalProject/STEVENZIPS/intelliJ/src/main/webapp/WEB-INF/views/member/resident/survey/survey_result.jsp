...
<head>
  ...
  <style>
    ...
    .my-answer-box {border:1px solid #dfe8df;background:#f8fbf8;border-radius:12px;padding:16px 18px;}
    .my-answer-label {font-size:12px;font-weight:800;color:var(--green-dark);margin-bottom:8px;}
    .my-answer-content {font-size:14px;line-height:1.8;color:var(--text-dark);white-space:pre-wrap;}
...
<body>
    ...
    <main class="content-area">
      <div class="page-content-wrap">
        <div class="breadcrumb">
          ...
          <span>›</span>
          <a href="javascript:void(0);">전자투표 및 설문</a>
          <span>›</span>
          <span class="cur">설문조사 - 결과</span>
        </div>
        <h1 class="page-title">설문조사 - 결과</h1>
        <p class="page-desc">설문 결과 조회와 목록 복귀 버튼을 구성한 설문조사 결과 화면입니다.</p>
        ...
        <div class="btn-row">
          <a class="btn-ghost"
             href="${pageContext.request.contextPath}/resident/survey/list/${survey.aptCd}">
  ...
  <script>
    ...
    <%-- 전체 참여자 수는 현재 사용자 응답(myResponse)이 아니라 설문 결과 기준으로 가져온다. --%>
    <%
      kr.or.ddit.domain.apt.mgmtOffice.survey.service.IMngSurveyService surveyServiceBean =
          org.springframework.web.context.support.WebApplicationContextUtils
              .getWebApplicationContext(pageContext.getServletContext())
              .getBean(kr.or.ddit.domain.apt.mgmtOffice.survey.service.IMngSurveyService.class);

      java.util.Map<String, Object> surveyResultData =
          surveyServiceBean.selectSurveyResult(
              ((kr.or.ddit.domain.apt.mgmtOffice.survey.dto.MngSurveyDTO) request.getAttribute("survey"))
                  .getSurveyNo()
          );

      int totalParticipantCount = 0;
      Object participantCountValue = surveyResultData.get("participantCount");
      if (participantCountValue instanceof Number) {
        totalParticipantCount = ((Number) participantCountValue).intValue();
      } else if (participantCountValue != null) {
        try {
          totalParticipantCount = Integer.parseInt(participantCountValue.toString());
...
((kr.or.ddit.domain.apt.mgmtOffice.survey.dto.MngSurveyDTO)
request.getAttribute("survey")).getSurveyQitemCn()
) %>';

    const questionList = JSON.parse(rawJson);
    ...
    const statistics =
            JSON.parse(
                    '<%= org.apache.commons.lang3.StringEscapeUtils.escapeEcmaScript(
                        String.valueOf(request.getAttribute("statisticsJson"))
                ) %>'
            );
    ...
    const myResponse =
            JSON.parse(
                    '<%= org.apache.commons.lang3.StringEscapeUtils.escapeEcmaScript(
                        String.valueOf(request.getAttribute("myResponse"))
                    ) %>'
            );
    ...
    function getMyAnswer(questionNo){

      const matched =
              myResponse.find(
                      r => Number(r.questionNo) === questionNo
              );
    ...
    questionList.forEach(function (q, idx){
      ...
      html +=
              "<div style='background:#edf5ef;color:var(--green-dark);padding:8px 14px;border-radius:999px;font-size:13px;font-weight:800;'>";
      ...
      q.items.forEach(function(item, rank){
        ...
        html +=
                "<div style='font-weight:800;color:var(--green-dark);'>";
        ...
        html +=
                "<div style='height:10px;background:#edf1ec;border-radius:999px;overflow:hidden;'>";

        html +=
                "<div style='height:100%;width:"
                + percent
                + "%;background:var(--green-dark);'></div>";
      ...
      if(q.type === "SHORT" || q.type === "LONG"){
        ...
        html +=
                ...
                + "<div class='my-answer-label'>내 답변</div>"
                + "<div class='my-answer-content'>"
                + escapeHtml(myAnswer).replaceAll("\n", "<br>")
                + "</div>"
                + "</div>";
      }
