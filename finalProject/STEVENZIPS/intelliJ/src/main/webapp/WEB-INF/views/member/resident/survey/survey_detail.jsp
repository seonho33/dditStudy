...
<body>
    ...
    <main class="content-area">
      <div class="page-content-wrap">
        <div class="breadcrumb">
          ...
          <span>›</span>
          <span class="cur">설문조사 - 상세</span>
        </div>
        <h1 class="page-title">설문조사 - 상세</h1>
        <p class="page-desc">설문 질문과 선택지, 설문 제출 및 목록 버튼을 포함한 설문조사 상세 화면입니다.</p>

      <section class="panel"><div class="section-hd"><h3>${survey.surveyNm}</h3><c:choose>

        <c:when test="${survey.surveySttsCd eq 'OPEN'}">
          <span class="badge ok">진행중</span>
        </c:when>

        <c:when test="${survey.surveySttsCd eq 'READY'}">
          <span class="badge wait">대기중</span>
        </c:when>

        <c:otherwise>
          <span class="badge info">종료</span>
        </c:otherwise>
        ...
        <div class="card">
          ...
          <div style="margin-top:12px; color:var(--text-light);">
        ...
        <div class="btn-row">
          ...
          <c:choose>
            ...
            <c:otherwise>
              ...
              <button type="button"
                      class="btn-sub"
                      onclick="alert('현재 참여할 수 없는 설문입니다.');">
        ...
        <a class="btn-ghost"href="${pageContext.request.contextPath}/resident/survey/list/${survey.aptCd}">목록으로</a>
        </div>
      </section>
  ...
  <script>
    ...
    const rawJson = `<%= ((kr.or.ddit.domain.apt.mgmtOffice.survey.dto.MngSurveyDTO)request.getAttribute("survey")).getSurveyQitemCn().replace("`","\\`") %>`;
    console.log(rawJson);
    ...
    const questionArea =
      ...
      questionList.forEach((q, idx) => {
        ...
        html += "<strong>";
        html += "Q" + (idx + 1) + ". " + q.question;
        html += "</strong>";

        html += "<div style='margin-top:20px;'>";

        if (q.type === "MULTI" || q.type === "YN") {
      ...
      submitBtn.addEventListener("click", async function () {
              ...
              questionList.forEach((q, idx) => {
                ...
                if(q.type === "SHORT"){
                  ...
                  const input =
                          document.querySelector(
                                  '.fake-input[data-question-no="' + questionNo + '"]'
                          );
                ...
                if(q.type === "LONG"){
                  ...
                  const textarea =
                          document.querySelector(
                                  '.fake-textarea[data-question-no="' + questionNo + '"]'
                          );
                ...
                if(!answer && invalidQuestionNo === null){
                  invalidQuestionNo = questionNo;
                }
              ...
              if(invalidQuestionNo !== null){
                alert("Q" + invalidQuestionNo + " 답변을 입력해주세요.");
                return;
              }
              ...
              try{
                ...
                if(!submitResponse.ok){
                  ...
                  if(message === "already"){
                    await residentAlert("이미 참여한 설문입니다.", "warning");
                  } else {
                    await residentAlert("설문 제출에 실패했습니다.", "error");
                  }
                ...
                await residentAlert("설문 제출 실패", "error");
              }
            });
