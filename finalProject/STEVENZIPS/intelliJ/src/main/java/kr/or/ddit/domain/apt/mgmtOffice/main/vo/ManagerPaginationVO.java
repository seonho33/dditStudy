...
@Data
public class ManagerPaginationVO<T> {

    private int totalRecord;     // 총 게시글 수
    private int totalPage;       // 총 페이지 수
    private int currentPage;     // 현재 페이지
    private int screenSize = 8;  // 페이지 당 게시글 수
    ...
    private String sidoNm;       // 시도 필터
    private String sigunguNm;    // 시군구 필터


    // ManagerPaginationVO 객체를 만들 때, 한 페이지당 게시글 수와 페이지 블록 수를 원하는 값으로
    // 초기화 할 수 있습니다.
    public ManagerPaginationVO() {}

    public ManagerPaginationVO(int screenSize, int blockSize) {
    ...
    public void setCurrentPage(int currentPage) {
        ...
        startPage = endPage - (blockSize - 1); // 시작 페이지 = 끝 페이지 - (블록 사이즈 - 1)
    }
