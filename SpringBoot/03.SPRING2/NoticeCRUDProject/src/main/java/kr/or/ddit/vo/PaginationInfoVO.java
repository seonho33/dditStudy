...
public class PaginationInfoVO<T> {
	...
	public PaginationInfoVO(int screenSize) {
		this.screenSize=screenSize;
	}
	
	public void setTotalRecord(int totalRecord) {
		// 총 게시글 수를 저장하고, 총 게시글 수를 페이지 당 나타낼 게시글 수로 나눠 총 페이지수를 구합니다.
		this.totalRecord = totalRecord;
		
		// 총 페이지수 = 총 게시글 수 / 1 페이지당 보여줄 게시글 수
		totalPage = (int)Math.ceil(totalRecord/(double)screenSize);
	}
	
	public void setCurrentPage(int currentPage) {
		this.currentPage = currentPage;	// 현재 페이지 저장
		//startRow, endRow, startPage, endPage currentPage 값을 활용해 자동 설정
		...
		// startPage, endPage는 blockSize 의 값을 활용해서 공식화
		// 마지막 페이지  = (현재페이지 + (블록사이즈-1)) / 블록사이즈 *블록사이즈
		// / blockSize* blockSize 는 1,2,3,4,5 .. 페이지마다 실수 계산이 아닌 정수 계산을 이용해
		// endPage를 구하기 위함
		endPage = (currentPage + (blockSize -1)) / blockSize * blockSize;
		startPage = endPage - (blockSize - 1);	// 시작페이지 = 끝 페이지 - (블록 사이즈 -1)
	}
	...
	public String getPagingHTML() {
		...
		// 현재 페이지가 1-5 사이의 범주에 있다면 startPage는 무조건 1이 됩니다.
		// 현재 페이지가 blockSize 보다 큰 6~범위에 있을 때 부터 startPage는 blockSize보다 큰 6부터
		// 시작합니다. 그런 점을 고려한다면, 현재 페이지 1보다 무조건 다음페이지에 있어야만 'Prev' 버튼을
		// 활용할 수 있으므로 조건식을 아래와 같이 작성 할 수 있습니다.
		if(startPage > 1) {
		...
		// 총 페이지가 14개고 현재 페이지가 9 페이지라면 넘어가야 할 페이지가 남아있는 것이기 때문에 endPage
		// 만큼 반복되고 넘어가야할 페이지가 존재하지 않는 상태라면 마지막 페이지가 포함되어 있는 block 영역
