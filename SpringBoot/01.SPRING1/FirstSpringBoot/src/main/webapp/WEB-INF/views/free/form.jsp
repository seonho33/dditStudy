	...
	<div class="container">
		<div class="row">
			<div class="col-md-12">
				<div class="card">
					<div class="card-body">
						<form method="post" action="/free/insert.do" id="freeInsertForm" class="form-horizontal">
							<c:if test="${update eq 'update' }">
								<input type="hidden" value="${freeVO.freeNo}" name="freeNo">
							</c:if>
							<div class="form-group row">
								<label class="col-sm-2 control-label" >제목</label>
								<div class="col-sm-10">
									<input id="freeTitle" name="freeTitle" type="text" class="form-control" value="${freeVO.freeTitle }" placeholder="제목을 입력해주세요.">
								</div>
							</div>
							<div class="form-group row mt-4">
								<label class="col-sm-2 control-label" >내용</label>
								<div class="col-sm-10">
									<textarea id="freeContent" name="freeContent" cols="50" rows="5" class="form-control" placeholder="content">${freeVO.freeContent }</textarea>
								</div>
							</div>
							<div class="form-group row mt-4">
...
$(function(){
	...
	let addBtn = $("#addBtn");
	let freeInsertForm = $("#freeInsertForm");
	let listBtn = $("#listBtn");
	let cancelBtn = $("#cancelBtn");
	
	addBtn.on("click",function(){
		let title = $("#freeTitle").val();
		let content = CKEDITOR.instances.freeContent.getData();
		...
		if(${update eq "update"}){
			freeInsertForm.attr("action","/free/update.do");
		}
		
		freeInsertForm.submit();
	})
	
	cancelBtn.on("click",function(){
		location.href="/free/detail.do?freeNo=${freeVO.freeNo}";
	})
