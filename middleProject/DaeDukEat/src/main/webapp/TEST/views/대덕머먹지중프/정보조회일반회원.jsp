        ...
        <div class="relative z-10 flex items-center gap-8">
            <div class="relative group">
                <div class="w-32 h-32 rounded-[35px] overflow-hidden border-4 border-white shadow-xl bg-white flex items-center justify-center">
                    <c:choose>
                        <c:when test="${not empty userVO.mem_img}">
                            <img src="${userVO.mem_img}" id="profile-img-preview" class="w-full h-full object-cover">
                        </c:when>
                        <c:otherwise>
                ...
                <button onclick="document.get ElementById('profile-upload').click()" 
            ...
            <div class="flex-1">
                ...
                <h3 class="text-4xl font-black text-slate-800 mb-2">
                    ${userVO.mem_name} <span class="text-lg font-bold text-orange-500 ml-1">님</span>
                </h3>
                <div class="flex gap-4">
                    <p class="text-slate-500 font-bold">계정 ID : <span class="text-slate-800">${userVO.mem_id}</span></p>
                    <div class="w-[1px] h-4 bg-orange-200 self-center"></div>
                    <p class="text-slate-500 font-bold">소속 : <span class="text-orange-600 font-black">${userVO.mem_num}기</span></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-6">
        <div class="bg-white border-2 border-slate-50 rounded-3xl p-8 shadow-sm flex justify-between items-center">
            <div>
                ...
                <p class="text-2xl font-black text-slate-700">${userVO.mem_bir}</p>
            </div>
        ...
        <div class="bg-white border-2 border-slate-50 rounded-3xl p-8 shadow-sm flex justify-between items-center">
            <div>
                ...
                <p class="text-2xl font-black text-slate-700">${userVO.mem_mail}</p>
            </div>
