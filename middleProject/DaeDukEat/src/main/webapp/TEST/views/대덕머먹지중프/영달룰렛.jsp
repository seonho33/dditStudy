        ...
        <div class="flex items-center gap-10">
            <div class="flex flex-col">
                ...
                <div class="flex items-center gap-6">
                    <input type="range" id="star-filter" min="1.0" max="5.0" step="0.5" value="3.0" oninput="document.getElementById('star-val').innerText = this.value">
                    <span id="star-val" class="text-4xl b-grade-font text-white w-14">3.0</span>
                </div>
            </div>
        </div>
        <div class="flex gap-6">
            <button onclick="addSlot()" class="border-2 border-orange-500 text-orange-500 px-8 py-4 rounded-full font-black text-lg hover:bg-orange-500 hover:text-white transition-all">슬롯 추가</button>
            <button onclick="spinAll()" class="bg-gradient-to-r from-orange-600 to-yellow-500 text-white px-20 py-4 rounded-full font-black text-3xl shadow-[0_0_30px_rgba(234,88,12,0.5)] active:scale-95 transition-all">JACKPOT GO!</button>
        </div>
    </footer>

    <script>
        // --- 1. [핵심] DB 데이터 자바스크립트 연동 (No-AJAX) ---
        // 서버에서 보낸 List<ShopVO>를 자바스크립트 배열로 변환
        const restaurants = [
            <c:forEach var="shop" items="${shopList}" varStatus="status">
            {
                category: "${shop.category}", // 카테고리 (한식, 중식 등)
                name: "${shop.name}",         // 식당 이름
                star: ${shop.starAvg},        // 별점 평균
                reviewCount: ${shop.reviewCount}, // 별점 개수 (추가 요청 반영)
                img: "${shop.img1}"           // 식당 이미지
            }${!status.last ? ',' : ''}
            </c:forEach>
        ];
        ...
        class Point {
            constructor() { this.x = Math.random() * canvas.width; this.y = Math.random() * canvas.height; this.baseX = this.x; this.baseY = this.y; this.size = Math.random() * 2 + 1; this.density = (Math.random() * 30) + 1; }
            update() {
            ...
            draw() { ctx.fillStyle = 'rgba(255, 157, 0, 0.7)'; ctx.beginPath(); ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2); ctx.fill(); }
        }
        function init() { points = []; for (let i = 0; i < 80; i++) points.push(new Point()); }
        function animate() {
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            for (let i = 0; i < points.length; i++) {
                points[i].update(); points[i].draw();
                for (let j = i; j < points.length; j++) {
                    let dx = points[i].x - points[j].x; let dy = points[i].y - points[j].y;
                    let dist = Math.sqrt(dx * dx + dy * dy);
                    if (dist < 120) { ctx.strokeStyle = `rgba(255, 157, 0, ${1 - dist/120})`; ctx.lineWidth = 0.5; ctx.beginPath(); ctx.moveTo(points[i].x, points[i].y); ctx.lineTo(points[j].x, points[j].y); ctx.stroke(); }
                }
            }
        ...
        function addSlot() {
            ...
            div.innerHTML = `
                ...
                <button onclick="this.parentElement.remove()" class="absolute -top-4 -right-4 bg-red-600 w-12 h-12 rounded-full font-black text-white opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">✕</button>
            `;
        ...
        function spinOne(id) {
            ...
            const minStar = parseFloat(document.getElementById('star-filter').value);
            const selectEl = container.closest('.group').querySelector('.category-select');
            const category = selectEl.value;

            // [필터 로직] DB 데이터 기반 필터링
            let filtered = restaurants.filter(r => r.star >= minStar);
            ...
            const spinCount = 40; // 회전 수 증가 (더 박진감 있게)
            let html = '';
            for (let i = 0; i <= spinCount; i++) {
                const r = filtered[i % filtered.length];
                html += `
                    <div class="slot-item">
                        <img src="${r.img}" onerror="this.src='https://via.placeholder.com/380x550/222/ff6b35?text=${r.name}'">
                        <div class="restaurant-info">
                            ...
                            <div class="flex justify-center items-center gap-3">
                                <span class="text-3xl text-yellow-400 font-black">★ ${r.star.toFixed(1)}</span>
                                <span class="text-gray-400 text-lg font-bold">(${r.reviewCount}개)</span>
                            </div>
                        </div>
                    </div>`;
            }
        ...
        function spinAll() {
            document.querySelectorAll('.slot-container').forEach(c => spinOne(c.id.replace('container-', '')));
        }
