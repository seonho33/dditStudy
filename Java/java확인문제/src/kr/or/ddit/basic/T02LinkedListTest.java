...
public class T02LinkedListTest {
	...
	/*
	    ...
	    Deque => 양끝의 어느 쪽에서든 데이터의 입출력이 가능한 
	             자료구조(ex: ArrayDeque) 	
	*/
		...
		/*
		    ...
		    1) 데이터 입력: push(저장할값)
		    2) 데이터 꺼내기: pop() => 데이터를 꺼내온 후 stack
		                          에서 데이터를 삭제한다. 
		*/
		stack.push("홍길동");
		stack.push("일지매");
		stack.push("변학도");
		stack.push("강감찬");
		System.out.println("현재 stack안의 데이터들 : "
				+ stack);
		...
		/*
		   ...
		   1) 데이터 입력: offer(저장할값)
		   2) 데이터 꺼내기: poll() => 데이터를 꺼내온 후 Queue에서
		                            제거한다. 
		*/
		queue.offer("홍길동");
		queue.offer("일지매");
		...
		System.out.println("현재 Queue안의 데이터 : " 
				+ queue);
		...
		if(queue.offer("성춘향")) {
			System.out.println("신규 데이터 등록 : 성춘향");
		}
		System.out.println("현재 Queue안의 데이터 : " + queue);
		System.out.println("꺼내온 데이터 : " + queue.poll());
