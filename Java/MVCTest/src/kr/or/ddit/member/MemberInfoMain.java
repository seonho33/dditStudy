...
public class MemberInfoMain {
	...
	/*
		검색할 회원 ID, 회원이름, 전화번호, 주소등을 입력하면
		입력한 정보만 사용하여 거맷ㄱ하는 기능을 구현하시오.
		주소는 입력한 값이 포함만 되어도 검색되도록 구현한다.
		입력하지 않을 정보는 엔터키로 다음 입력으로 넘긴다.
	*/
		scan.nextLine(); //입력 버퍼 비우기
		System.out.println();
		System.out.println("검색할 회원 Id를 입력해 주세요 ");
		...
		System.out.print("주소 >> ");
		String memAddrs = scan.nextLine().trim();
		
		MemberVO mv = new MemberVO(memId, memName, memTel, memAddrs);
		...
		System.out.println("----------------------------------");
		System.out.println("전체 회원정보 조회 완료...");
