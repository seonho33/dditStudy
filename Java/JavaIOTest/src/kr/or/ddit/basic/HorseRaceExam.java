...
public class HorseRaceExam {
	...
	public static void main(String[] args) {
		...
		horseList = Arrays.asList(new Horse("01번말"), new Horse("02번말"), new Horse("03번말"), new Horse("04번말"),
				...
				new Horse("10번말"));
		//horseList.add(new Horse("99번말")); // UnsupportedOperationException 발생 => 리스트 항목을 추가 및 삭제 못하도록 처리.
		...
		System.out.println("(3초 뒤에 순위 결과가 발표됩니다.)");
		System.out.println();
		System.out.println();
		System.out.println();
...
class Horse extends Thread implements Comparable<Horse> {
	...
	public void run() {
		for (int i = 0; i < 50; i++) {
			...
			setHorsePosi(i); // 각 구간으로 말의 위치 이동(설정)
			
			displayPosition(); // 현재 말 위치를 출력하기
		}
	...
	private void displayPosition() {
		...
		if (this.getHorsePosi() != 49) { // 아직 도착하지 않은 경우...
			...
			System.out.print(horseCourse.substring(this.getHorsePosi()+1, 50));
	...
	/**
	 *  현재 말위치를 화면에 출력하기 위한 메서드2
	 */
	private void displayPosition2() {
		
		System.out.print(this.getHorseName() + " : ");
		
		if (this.getHorsePosi() != 49) { // 아직 도착하지 않은 경우...
			for(int i = 0; i< 50; i++) {
				if(i == this.getHorsePosi()) {
					System.out.print(">");
				}else {
					System.out.print("-");
				}
...
class HorsePositionDisplay extends Thread {
	...
	/**
	 * 깔끔한 화면 출력을 위한 빈줄 출력 메서드
	 */
	public void printEmptyLine(int lineCnt) {
	...
	public void run() {
		...
		while (true) {
			...
			printEmptyLine(30); // 화면 정리를 위해 빈줄 출력하기
